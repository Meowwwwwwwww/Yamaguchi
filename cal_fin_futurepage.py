...
class future_value_page(object):
    ...
    def create_page(self):
        ...
        Label(self.page, text='复利终值计算:           ').grid(row=0, stick=W, pady=10)
        ...
        Label(self.page, text='现值').grid(row=4, column=0, stick=W, pady=10)
        Entry(self.page, textvariable=self.pv).grid(row=4, column=1, stick=E)
    ...
    def calculate(self):
        if (self.rate.get() == '' or self.nper.get() == '' or self.pmt.get() == '' or self.pv.get() == ''):
        ...
        self.result.set('%.8f'%np.fv(rate, nper, pmt, pv, when = 'end'))
